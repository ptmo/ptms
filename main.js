...
async function submitPost() {
    ...
    if (typeof pendingNftData !== 'undefined' && pendingNftData) {
        try {
            ...
            showToast("3/3 Konfirmasi di Wallet...", "info");
            ...
            showToast("Menunggu konfirmasi blok...", "info");
            await tx.wait();
            
            // --- SUKSES ---
            showToast("Berhasil Mengarsipkan Wayang!", "success");
            
            // BERSIHKAN FORM (Dengan Pengecekan Aman)
            if (typeof cancelNft === "function") {
                cancelNft(); 
            } else {
            ...
            // AUTO REFRESH AGAR POSTINGAN MUNCUL
            setTimeout(() => {
                window.location.reload();
            }, 1000); // Tunggu 1 detik agar user sempat baca toast sukses

        } catch (e) {
            console.error(e);
            // Hanya tampilkan error jika bukan user reject
...
// --- FUNGSI RESET FORM NFT ---
function cancelNft() {

    // 2. Sembunyikan Box Hijau "Data Siap!"
    // (Pastikan ID 'nftDraftBox' atau div pembungkus pesan hijaunya benar)
    // Jika Anda tidak tahu ID-nya, kode ini akan mencari elemen alert hijau secara otomatis
    const draftBox = document.querySelector(".alert-success") || document.getElementById("nftDraftBox");
    if (draftBox) {
        draftBox.style.display = "none"; 
        // Atau hapus isinya jika dibuat dinamis
        draftBox.innerHTML = "";
    }

    // 3. Reset Input File (Opsional, agar bersih)
    const fileInput = document.getElementById("fileInput"); // Sesuaikan ID input file Anda
